...
def get_ammo(calibre):
    ...
    for i in bullet_list:
        ...
        bullet_dict.append({'Name': par_list[0].text, 'Damage': par_list[1].text, 'Penetration power': par_list[2].text,
                            'Armor damage': par_list[3].text, "Accuracy": par_list[4].text, "Recoil": par_list[5].text,
                            "Frag.": par_list[6].text, '1': par_list[9].text, '2': par_list[10].text,
                            '3': par_list[11].text, '4': par_list[12].text, '5': par_list[13].text,
                            '6': par_list[14].text})
...
def get_market_items(item_name):
    driver = webdriver.Chrome(executable_path="D:\driver\chromedriver.exe")
    driver.get("https://tarkov-market.com/ru/")
    input_line = driver.find_element_by_tag_name("input")
    input_line.send_keys(item_name)
    time.sleep(3)
    cards = driver.find_elements_by_class_name("card")
    output_dict = []
    for i in cards:
        output_dict.append({"Name": i.find_element_by_tag_name('a').text, "Price": i.find_element_by_class_name("alt.big").text, "Src": i.find_element_by_tag_name("img").get_attribute("src")})
    driver.close()
